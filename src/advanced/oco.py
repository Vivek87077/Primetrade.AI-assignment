...
def place_oco_order(api_key, api_secret, symbol, side, quantity, price, stop_price, stop_limit_price):
    ...
    try:
        ...
        print(f"OCO order placed: {order}")
    except BinanceAPIException as e:
        logging.error(f"Error placing OCO order: {e}")
        print(f"Error: {e}")

def main():
    parser = argparse.ArgumentParser(description='Place an OCO order on Binance Futures.')
    parser.add_argument('symbol', help='Trading pair, e.g., BTCUSDT')
    parser.add_argument('side', choices=['BUY', 'SELL'], help='Order side')
    parser.add_argument('quantity', help='Order quantity')
    parser.add_argument('price', help='Take profit price')
    parser.add_argument('stop_price', help='Stop price')
    parser.add_argument('stop_limit_price', help='Stop limit price')
    ...
    place_oco_order(api_key, api_secret, args.symbol, args.side, quantity, price, stop_price, stop_limit_price)
